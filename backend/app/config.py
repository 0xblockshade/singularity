...
DATA_DIR = os.environ.get("INFINITUM_DATA", os.path.join(BACKEND_DIR, "data"))
DB_PATH = os.environ.get("INFINITUM_DB", os.path.join(DATA_DIR, "infinitum.db"))
...
NARRATOR_MODEL = os.environ.get("INFINITUM_NARRATOR_MODEL", "claude-opus-5")
TRIAGE_MODEL = os.environ.get("INFINITUM_TRIAGE_MODEL", "claude-haiku-4-5")
NARRATOR_MAX_TOKENS = int(os.environ.get("INFINITUM_MAX_TOKENS", "16000"))

# Inbox guards (metadata only — the agent still reads everything; see DESIGN.md §5).
RATE_LIMIT_PER_HOUR = int(os.environ.get("INFINITUM_RATE_LIMIT", "30"))
DEDUP_WINDOW_MIN = int(os.environ.get("INFINITUM_DEDUP_WINDOW_MIN", "60"))
MAX_TRANSMISSION_CHARS = int(os.environ.get("INFINITUM_MAX_TX_CHARS", "4000"))

# How many signals / transmissions the ritual feeds the narrator per wake.
MAX_SIGNALS_PER_RUN = int(os.environ.get("INFINITUM_MAX_SIGNALS", "60"))
MAX_TRANSMISSIONS_PER_RUN = int(os.environ.get("INFINITUM_MAX_TX", "200"))
...
SANDBOX_RESEARCH_SIGNALS = int(os.environ.get("INFINITUM_SANDBOX_SIGNALS", "25"))
...
def live_scan_enabled() -> bool:
    ...
    return _truthy(os.environ.get("INFINITUM_LIVE_SCAN"))


# Per-adapter cap on items pulled per wake. Feed / subreddit lists live in the adapters.
ADAPTER_MAX_RESULTS = int(os.environ.get("INFINITUM_ADAPTER_MAX", "15"))

# Shared HTTP knobs. Reddit's public JSON and some feeds reject a default UA, so set one.
HTTP_TIMEOUT = float(os.environ.get("INFINITUM_HTTP_TIMEOUT", "15"))
HTTP_USER_AGENT = os.environ.get(
    "INFINITUM_USER_AGENT",
    "infinitum-narrator/0.1 (autonomous AI-progress observer)",
)

# Key-gated adapters. Absent the key, the adapter cleanly returns [] (never crashes).
WEBSEARCH_KEY = os.environ.get("INFINITUM_WEBSEARCH_KEY", "")
WEBSEARCH_URL = os.environ.get("INFINITUM_WEBSEARCH_URL", "https://api.tavily.com/search")
X_KEY = os.environ.get("INFINITUM_X_KEY", "")
X_API_URL = os.environ.get("INFINITUM_X_API_URL", "")
X_QUERY = os.environ.get("INFINITUM_X_QUERY", "AGI OR superintelligence OR frontier model")
...
TRIAGE_ENABLED = _truthy(os.environ.get("INFINITUM_TRIAGE"))
...
PUBLISH_X = os.environ.get("INFINITUM_PUBLISH_X", "").lower() in ("1", "true", "yes", "on")
X_API_KEY = os.environ.get("X_API_KEY", "")
X_API_SECRET = os.environ.get("X_API_SECRET", "")
X_ACCESS_TOKEN = os.environ.get("X_ACCESS_TOKEN", "")
X_ACCESS_SECRET = os.environ.get("X_ACCESS_SECRET", "")
# Public site URL, used to build the link back to the full dispatch in each post.
PUBLIC_URL = os.environ.get("INFINITUM_PUBLIC_URL", "").rstrip("/")
